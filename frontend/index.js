const operation_name_map = {
  ...
  document.querySelector('input[type="file"]').addEventListener('change', function (event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      let options = ['Edge Detection', 'Corner Detection', 'Gaussian Blur', 'Median Blur', 'Fourier Transform', 'Contrast Enhancement', 'Background Removal'];
  ...
  document.getElementById('uploadForm').addEventListener('submit', function (event) {
    ...
    Array.from(files).forEach((file, i) => {
      ...
      function sendRequest() {
        fetch('http://Web-app-and-server-413737600.eu-north-1.elb.amazonaws.com:8000/process-image        ', {
          method: 'POST',
          body: formData
        })
          .then(response => {
          ...
          .catch(error => {
            ...
            if (requestCount < REQUEST_NUMBER) {
              ...
              alert('Processing Image number '+ Number(i+1) + " Failed");
            }
          });
      }
